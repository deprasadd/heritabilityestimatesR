...
sort(blood$VG.Vp.ratio.SE)

data = blood

se=format(round(blood$VG.Vp.ratio.SE, 2), nsmall = 2)
vg = format(round(blood$VG.Vp.ratio.var, 2), nsmall = 2)
vg=paste(vg, "(", se,")",  sep = "")
...
p = ggplot(data = blood, aes(x = as.factor(num_data),y = blood$VG.Vp.ratio.var, reorder(blood$VG.Vp.ratio.SE, -value))) + 
  ...
  geom_hline(yintercept = 0)+
   ...
   geom_bar(aes(fill=blood$Pval), stat = "identity")+
  ylim(-1,1)+
  theme_minimal()+
  theme(
  ...
  geom_text(x=0, y=-1, label="Heritability Estimates of Blood-Based Biomarkers", size=4)+
...
p
ggsave(filename = "blood.jpg", p, width = 25.6, height = 14.4, units = "in", dpi=800)
