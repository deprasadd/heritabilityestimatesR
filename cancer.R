...
sort(cancer$VG.Vp.ratio.SE)

data = cancer

se=format(round(cancer$VG.Vp.ratio.SE, 2), nsmall = 2)
vg = format(round(cancer$VG.Vp.ratio.var, 2), nsmall = 2)
vg=paste(vg, "(", se,")",  sep = "")
...
p = ggplot(data = cancer, aes(x = as.factor(num_data),y = cancer$VG.Vp.ratio.var, reorder(cancer$VG.Vp.ratio.SE, -value))) + 
  ...
  geom_bar(aes(fill=cancer$Pval), stat = "identity")+
  ylim(-1,1)+
  theme_minimal()+
  theme(
  ...
  geom_text(data=label_data, aes(x=num_data, y=cancer$VG.Vp.ratio.var+.02, label=cancer$lab.name, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=3, angle= label_data$angle, inherit.aes = FALSE )+
  geom_text(x=0, y=-1, label="Heritability Estimates of Cancer-Based Biomarkers", size=4)+
...
p
ggsave(filename = "cancer.jpg", p, width = 25.6, height = 14.4, units = "in", dpi=800)
